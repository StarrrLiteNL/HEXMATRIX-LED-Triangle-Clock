...
Ticker tkBrightness;										  // Second - Timer for Updating brightness
//custom declarations
...
struct strConfig {
  ...
  int  brightnessLow;                   // 16 byte - EEPROM 192
  int  brightnessHigh;                  // 16 byte - EEPROM 208
  int  offsetSunrise;                   // 16 byte - EEPROM 224
  int  offsetSunset;                    // 16 byte - EEPROM 240
  int  spreadSunrise;                   // 16 byte - EEPROM 256
  int  spreadSunset;                    // 16 byte - EEPROM 282
  // Application Settings here... EEPROM up to 511 (0 - 511)
} config;
...
void WriteConfig(){
  ...
  EEPROM.write(192, config.brightnessLow);
  EEPROM.write(208, config.brightnessHigh);
  EEPROM.write(224, config.offsetSunrise);
  EEPROM.write(240, config.offsetSunset);
  EEPROM.write(256, config.spreadSunrise);
  EEPROM.write(282, config.spreadSunset);
...
boolean ReadConfig(){
  ...
  if (EEPROM.read(0) == 'C' && EEPROM.read(1) == 'F'  && EEPROM.read(2) == 'G' )
  {
    ...
    config.brightnessLow  = EEPROM.read(192);
    config.brightnessHigh = EEPROM.read(208);
    config.offsetSunrise  = EEPROM.read(224);
    config.offsetSunset   = EEPROM.read(240);
    config.spreadSunrise  = EEPROM.read(256);
    config.spreadSunset   = EEPROM.read(282);
...
void printConfig(){
  ...
  Serial.printf("Sunrise offset:%d\n", config.offsetSunrise);
  Serial.printf("Sunrise spread:%d\n", config.spreadSunrise);
  
  Serial.printf("Sunset offset:%d\n", config.offsetSunset);
  Serial.printf("Sunset offset:%d\n", config.spreadSunset);

  Serial.printf("Brightness low:%d\n", config.brightnessLow);
  Serial.printf("Brightness high:%d\n", config.brightnessHigh);
