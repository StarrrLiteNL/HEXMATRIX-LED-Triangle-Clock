...
// sunset libray
#include <sunset.h>
#define LATITUDE        52.710011
#define LONGITUDE       5.753870
#define TIMEZONE		1
long sunRise = 480;
long sunSet = 1320;
SunSet sun;
...
void setup() {
	...
	if ((WIFI_connected != WL_CONNECTED) or !CFG_saved) {
		...
		config.brightnessLow = 60;
		config.brightnessHigh = 150;
		config.offsetSunrise = 0;
		config.offsetSunset = 0;
		config.spreadSunrise = 60;
		config.spreadSunset = 60;
		WriteConfig();
		WiFi.mode(WIFI_AP);
    ...
    server.on("/brightness.html", brightness_Set_html);
	server.on("/style.css", []() {
	...
	server.on("/admin/brightnessvalues", brightness_Set_values_html);
	server.onNotFound([]() {
	...
	// start internal time update ISR
	tkBrightness.attach(1, updateBrightness);
}
